  ...
  process(
    ...
    for (let i = 0; i < nbFireworks; i++) {
      ...
      */
      ...
      pokemon.simulation.context.addDelayedAction(1000 + 250 * i, () => {
          if (
            !pokemon.simulation ||
            !pokemon.simulation.context ||
...
export class MagnetPullStrategy extends AbilityStrategy {
  ...
  process(pokemon: PokemonEntity, board: Board, target: null, crit: boolean) {
    super.process(pokemon, board, target, crit)
    if (pokemon.player && pokemon.simulation.context) {
      const randomSteelPkm = pokemon.simulation.context.state.shop.magnetPull(
        pokemon,
        pokemon.player as Player
      )
      pokemon.simulation.context.spawnWanderingPokemon({
        pkm: randomSteelPkm,
