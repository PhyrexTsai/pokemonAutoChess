...
- What happens when a Pokemon spawns a clone/summon mid-battle? The Simulation handles entity creation internally; the spawned entity is visible to clients via Colyseus Schema auto-sync (no explicit BattleEvent needed).
...
- **FR-002**: The `Simulation` class MUST NOT REQUIRE a `GameRoom` to function. An optional `room?: GameRoom` reference is retained for 5 abilities (MIND_BLOWN, MAGNET_PULL, UNOWN F, UNOWN W, FIELD synergy) that silently skip when room is absent. Full removal deferred to Phase 2. See FR-007 for constructor signature details.
...
- **FR-007**: The `Simulation` constructor MUST accept `ISimulationPlayer` interfaces (narrow type satisfied by Player via structural typing), weather, stage level, game rules, ghost battle flag, and an optional `room?: GameRoom` as final parameter. The optional room enables 5 abilities (MIND_BLOWN, MAGNET_PULL, UNOWN F, UNOWN W, FIELD synergy) that access clock, shop, spawn, and evolution-check methods — silently skipped when absent. The class name `Simulation` is preserved; no renaming.
