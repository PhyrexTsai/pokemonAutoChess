...
- **FR-013**: All room references in core files (`simulation.ts`, `mini-game.ts`, `effect.ts`, `items.ts`, `passives.ts`, `synergies.ts`, `abilities.ts`, `hidden-power.ts` — 34 refs across 8 files) MUST be fully resolved by replacing `GameRoom` with `IGameEngineContext` interface — either reimplemented without room dependency or removed if the functionality is multiplayer-only.
