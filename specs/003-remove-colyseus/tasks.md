...
- [ ] T001 Move `app/rooms/states/game-state.ts` to `app/models/colyseus-models/game-state.ts` and update all import paths. Files to update: `app/core/matchmaking.ts`, `app/core/mini-game.ts`, `app/core/scribbles.ts`, `app/models/colyseus-models/pokemon.ts`, `app/models/colyseus-models/player.ts`, `app/models/shop.ts`, `app/public/src/game/game-container.ts`, `app/public/src/game/scenes/game-scene.ts`, `app/public/src/game/components/board-manager.ts`, `app/public/src/game/components/minigame-manager.ts`, `app/public/src/pages/game.tsx`, `app/public/src/pages/after-game.tsx`, `app/public/src/pages/lobby.tsx`, `app/public/src/network.ts`. Verify build passes.
- [ ] T002 Add `IGameEngineContext` interface to `app/types/index.ts` per data-model.md definition (state, addDelayedAction, emit, spawnOnBench, spawnWanderingPokemon, checkEvolutionsAfterPokemonAcquired, checkEvolutionsAfterItemAcquired, getTeamSize). Leave existing GameRoom import intact (still used by rooms/ files).
...
- [ ] T009 [P] Replace room refs in `app/core/effects/synergies.ts` — 2 refs: `room.clock.setTimeout` pattern → `context.addDelayedAction`. Update function signatures.
- [ ] T010 [P] Replace room refs in `app/core/abilities/abilities.ts` — 3 refs accessed via `pokemon.simulation.room`: `room.clock.setTimeout` (1×) → `context.addDelayedAction`, `room.state.shop` (1×) → `context.state.shop`, `room.spawnWanderingPokemon` (1×) → `context.spawnWanderingPokemon`. Change access pattern from `pokemon.simulation.room` to `pokemon.simulation.context`.
- [ ] T011 [P] Replace room refs in `app/core/abilities/hidden-power.ts` — 3 refs accessed via `unown.simulation.room`: `room.state.shop` (1×), `room.state` (1×), `room.spawnOnBench` (1×) → `context.state.shop`, `context.state`, `context.spawnOnBench`. Change access from `unown.simulation.room` to `unown.simulation.context`.
...
- [ ] T012 [P] Create `app/public/src/game-engine-commands.ts` (~1000 lines) — extract player action functions from `app/rooms/commands/game-commands.ts` as plain functions. Each function accepts `(state: GameState, player: Player, context: IGameEngineContext, params)` and mutates state in-place. Extract: buyPokemon, sellPokemon, rerollShop, levelUp, lockShop, dragDropPokemon, dragDropItem, dragDropCombine, pickPokemonProposition, pickItem, showEmote, pickBerry, wandererClicked, switchBenchAndBoard, removeFromShop, sellPokemonFromScene, reportLoadingProgress, reportLoadingComplete, sendVector. Replace all 42 `this.room` refs with context parameter, all 167 `this.state` refs with state parameter, all 25 `client.send()` calls with `context.emit()`. Delete multiplayer-only commands (OnJoinCommand, OnLeaveCommand, chat, spectate, tournament, admin).
...
- [ ] T016 [US1] Modify `app/public/src/game/game-container.ts` (~12 changes) — replace constructor param from `Room<GameState>` to `LocalGameEngine`. Replace `getStateCallbacks(room)` with `getDecoderStateCallbacks(engine.decoder)`. Replace `SchemaCallbackProxy` type with inferred type. Replace 4 `room.send(Transfer.*)` with engine method calls. Replace 1 `room.onMessage(Transfer.DRAG_DROP_CANCEL)` with `engine.on(Transfer.DRAG_DROP_CANCEL)`. Replace 3 `room.state` reads with `engine.clientState`. Remove `room.onError`. Remove `@colyseus/sdk` imports.
- [ ] T017 [US1] Modify `app/public/src/game/scenes/game-scene.ts` (~16 changes) — change `room` property type from `Room<GameState>` to `LocalGameEngine`. Replace 9 `this.room?.send(Transfer.*)` calls with engine method calls (LOADING_PROGRESS, LOADING_COMPLETE, LOCK, REFRESH, LEVEL_UP, SELL_POKEMON, REMOVE_FROM_SHOP, SWITCH_BENCH_AND_BOARD, VECTOR). Replace 5 `this.room.state` reads with `this.engine.clientState`. Replace 1 `this.room!.onMessage` with `this.engine.on`. Remove `Room` type import from `@colyseus/sdk`.
- [ ] T018 [US1] Modify `app/public/src/pages/game.tsx` (~30 changes) — replace `getStateCallbacks(room)` with `getDecoderStateCallbacks(engine.decoder)`. Replace 16 `room.onMessage(Transfer.*)` listeners with `engine.on(Transfer.*)`. Replace 7 `room.state` reads with `engine.clientState`. Remove 6 lifecycle refs (`room.leave()`, `room.onDrop()`, `room.onReconnect()`, `room.onLeave()`, `room.reconnectionToken`, `room.roomId`). Update GameContainer creation to pass engine. Remove `Room` type import from `@colyseus/sdk`. Remove `AfterGameState` import (no longer used for room creation).
...
- [ ] T022 [P] [US1] Modify `app/public/src/game/components/minigame-manager.ts` — replace 1 `room.onMessage(Transfer.NPC_DIALOG)` with `engine.on(Transfer.NPC_DIALOG)`.
